...
# Context-aware logging + Datadog DogStatsD (utils live under src/ in Lambda package)
try:
...
# Log critical errors for missing modules after logger is initialized
# stdlib: module import time — no lambda_handler / MikAura scope yet
if not PipelineInfoHelper:
    logger.error("CRITICAL: PipelineInfoHelper not available - pipeline-info table updates will fail")

if not STANDARDIZATION_AVAILABLE:
    logger.warning("Column standardization not available - data validation may be limited")

if not CONFIG_AVAILABLE:
    logger.warning("Pipeline configuration not available - using environment variable fallbacks")
...
def load_drift_profile() -> Optional[Dict[str, Any]]:
    """Load drift profile once per runtime; return None when unavailable."""
    # stdlib: first call may occur before per-invocation MikAura logger; keep ContextLogger
    ...
    if not profile_path:
        logger.warning("Spend regime shift profile not found - skipping ME-5401 checks for this run")
        _DRIFT_PROFILE_CACHE = None
        return None
    try:
        with open(profile_path, 'r', encoding='utf-8') as profile_file:
            _DRIFT_PROFILE_CACHE = json.load(profile_file)
            logger.info(f"Loaded spend regime shift profile from {profile_path}")
            return _DRIFT_PROFILE_CACHE
    except Exception as e:
        logger.warning(f"Failed to load spend regime shift profile from {profile_path}: {e}")
        _DRIFT_PROFILE_CACHE = None
        return None
...
def handle_spend_regime_shift_actions(
    ...
    if not PipelineInfoHelper:
        ...
        return
    try:
        helper = PipelineInfoHelper(ENVIRONMENT)
...
def send_schema_drift_alert(
    ...
    dropped_by_transform: List[str] = None,
    status_logger: Optional[Any] = None,
) -> bool:
    """
    Send a Slack alert when schema drift is detected (source differs from SOT).
    
    TASK 6: Enhanced to distinguish missing-in-source vs dropped-by-transform.
    ...
    Args:
        ...
        missing_columns: SOT columns missing from output (combined, for backward compatibility)
        extra_columns: Extra columns in source not in SOT
        filename: Source filename
        missing_in_source: TASK 6: SOT columns never present in source (optional)
        dropped_by_transform: TASK 6: SOT columns present in source but dropped during transform (optional)
    ...
    # TASK 6: Use classification if provided, otherwise fall back to combined missing_columns
    if missing_in_source is None:
        missing_in_source = []
    if dropped_by_transform is None:
    ...
    try:
        payload = {
            ...
            'brand_name': brand_name,
            'retailer_id': retailer_id,
            'missing_columns': missing_columns,  # Backward compatibility
...
def reorder_columns_sot(
    ...
    for col in df.columns:
        ...
        elif col_normalized.endswith('_impressions') and col_normalized not in media_channels_lower:
    ...
    try:
        ...
        if has_date_column:
            ...
            try:
                ...
                # Try to parse dates
                parsed_dates = pd.to_datetime(df[date_col_name], errors='coerce', dayfirst=True)
        ...
        for retailer_id in detected_retailers:
            ...
            extra_cols = [
                ...
                and not normalize_header_for_matching(col).endswith('_impressions')  # Exclude retailer impressions
            ]
    ...
    try:
        ...
        try:
            if PANDAS_AVAILABLE:
                if 'df' not in locals():
                    df = pd.read_csv(io.BytesIO(retailer_data))
                drift_profile = load_drift_profile()
                if drift_profile:
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    with ingestion_context(
        ...
        retailer_id=retailer_id or "",
        country=obs_country,
        use_case="data_ingestion_transfer",
        pipeline_component="data_transfer_lambda",
        execution_id=step_function_execution_id,
        data_source_bucket=TRACER_BUCKET,
    ):
