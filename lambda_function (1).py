...
# Dynamic drift profile builder — replaces static profile.json approach (ME-5401/ME-5402 fix)
...
try:
    ...
    DYNAMIC_DRIFT_AVAILABLE = False
    print("WARNING: dynamic_drift_profile not available - drift detection will be skipped", file=sys.stderr)
...
# Drift detection thresholds for ME-5401 (Spend Regime Shift) / ME-5402 (KPI Behavior Break)
...
def process_file(
    ...
    try:
        ...
        # Dynamic per-retailer drift detection (ME-5401 + ME-5402)
        # Replaces static profile.json approach — each retailer is now evaluated
        # against its own historical baseline built at runtime.
        try:
            if PANDAS_AVAILABLE and DYNAMIC_DRIFT_AVAILABLE:
                ...
                if not is_eligible:
                    _transfer_info(
                        status_logger,
                        f"Drift checks skipped for {client_id}/{brand_name}/{retailer_id}: "
                        f"insufficient history ({len(historical_df)} rows, need {MIN_ROWS_FOR_DRIFT})",
                        client_id=client_id,
                        brand_name=brand_name,
                        retailer_id=retailer_id,
                    )
                else:
                    # Dynamically infer schema from THIS retailer's columns
                    schema = infer_schema(df)

                    if schema.get('kpi_col') is None:
                        _transfer_warning(
                            status_logger,
                            f"Drift checks skipped: no KPI column detected for {retailer_id}",
                            client_id=client_id,
                            brand_name=brand_name,
                            retailer_id=retailer_id,
                        )
                    elif not schema.get('spend_cols'):
                        _transfer_warning(
                            status_logger,
                            f"Drift checks skipped: no spend columns detected for {retailer_id}",
                            client_id=client_id,
                            brand_name=brand_name,
                            retailer_id=retailer_id,
                        )
                    else:
                        # Build profile from THIS retailer's own historical data
                        drift_profile = build_reference_profile(historical_df, schema)

                        # ME-5401: Spend Regime Shift
                        spend_regime_result = detect_spend_regime_shift(df, drift_profile)
                        if spend_regime_result.get('detected'):
                            _transfer_warning(
                                status_logger,
                                f"ME-5401 spend regime shift detected for "
                                f"{client_id}/{brand_name}/{retailer_id}: "
            ...
            _transfer_warning(
                status_logger,
                f"Drift detection (ME-5401/ME-5402) failed for {file_key}: {drift_error}",
                file_key=file_key,
